...
private:
	XmlFilesDefinitions(){
		userPreferences.add(toList({"settings", "ext_converter_path"}), UserPreferences::EXT_CONVERTER_PATH);
		userPreferences.add(toList({"settings", "output_destination"}), UserPreferences::DESTINATION_PATH);
		userPreferences.add(toList({"settings", "number_of_cpus"}), UserPreferences::NUM_OF_CPU);
		userPreferences.add(toList({"settings", "encoder_multithread"}), UserPreferences::ENABLE_MULTITHREAD_FOR_ENC);
		//************************************************************************************************************
		profil.add(toList({"profile", "name"}),
				Profile::Profile::NAME_OPT);
		profil.add(toList({"profile", "container"}), Profile::Profile::CONTAINER_OPT);
